...
def index(request,cate=None):
    ...
    page_robot = Paginator(result_list, 12)
...
def publish_post(request):
    ...
    if form.is_valid():
        ...
        user = request.user
        article = Article(title=title, img=image, content=content,cate_choice=choice,author=user,price=price)
        article.save()
        page_id = article.id
...
def buy(request, page_num):
    ...
    if request.user.is_authenticated:
        ...
        if form.is_valid():
            ...
            buy =  Buy(name=name,buyer=buyer,address=address,phone=phone,article=article)
            buy.save()
            message = InstationMessage()
            message.sender = buyer
            message.receiver = article.author
            message.name = buy.name
            message.phone = buy.phone
            message.address =buy.address
            message.article = buy.article
            message.save()
            return redirect('detailbuy',page_num)
        else:
            return detail(request, page_num=page_num, error_form=form)

def detailbuy(request, page_num, error_form=None):
...
def display_message(request):
    ...
    messages =InstationMessage.objects.filter(receiver=user)
